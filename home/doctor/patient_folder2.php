   ...
   <div class="pahistory">
     ...
     */ 
     ...
     while ($row= mysqli_fetch_array($result)){
                ...
                <div style='border:2px solid red' class='band'><br>
      ...
      // if(isset($_GET["idd"])){
      // $sen =$_GET['idd'];
      $queryy = "SELECT * FROM consultation_annex_table  WHERE case_id  = {$id} ORDER BY update_date DESC";
     

          $res =mysqli_query($connect, $queryy);
          $query_row =mysqli_fetch_array($res);
           
          $namez =$query_row['patient_name'];
          $history =$query_row['clinical_history'];
          $medicinename =$query_row['prescription'];
          $bp =$query_row['bp'];
          $diagnosis =$query_row['diagnosis'];
          $doctor =$query_row['doc_name'];
          $update_date =$query_row['update_date'];
          $results='naming ceremoney';
          $spo2 =$query_row['spo2'];
          

       $query_result =mysqli_query($connect, $queryy);
      ...
      echo"<tr><td style=' border: 1pt double ;background:#3090C7;color:white;'></<td></tr>";
     }

     while ($row2= mysqli_fetch_array($query_result)){
                  $id = $row2['case_id'];
                  $name = $row2['patient_name'];
                  $history = $row2['clinical_history'];
                  $medicinename = $row2['prescription'];
                  $diagnosis = $row2['diagnosis'];
                  $lab =$row2['labs'];
                  $doctor =$row2['doc_name'];
                  $update_date =$row2['update_date'];
                  $allMedicines = json_decode($medicinename, true);
                  $drugg =$row2['prescription'];
                  $spo2 =$row2['spo2'];
                  // loop through the medicines and use it to created the nested table
                  // you can style the table however you like
                ...
                <div class='wan' style=color:green><br>
                    <div class='wann'>
                    

                    <div style='border:2px solid blue' class='band'><br>
               ...
               <a href='updatefolder.php?casecode={$casecode}&id=".$id." ' style='background:black;padding-top:5px;padding-bottom:5px; width:100%;color:white;text-decoration:none;padding-right:27px;padding-left:27px;border-radius:6px;'>Update 2</a>
               <a href='addmore.php?casecode={$casecode}&id=".$id." ' style='background:gold;padding-top:5px;padding-bottom:5px; width:100%;color:black;text-decoration:none;padding-right:15px;padding-left:15px;border-radius:6px;'>Add More 2</a>
     ...
     <!-- use this query to fetch newly added data and use the query_results to create the table -->
