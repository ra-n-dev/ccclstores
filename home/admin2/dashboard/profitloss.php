...
<div class="container-fluid">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 " id="profitloss" style=" width:100%">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h5 class="tees">Profit & Loss</h5>
    ...
    <div class="row">
        ...
        <div class="row" style="margin-left:0.2%;margin-bottom: 2%;">
            <div class="card border-left-primary shadow h-100 py-2 studdlist">
                ...
                <pre></pre>
                <h5 class="payroll">
                    <span>PROFIT & LOSS SCHEDULE </span><br>
                    <span>FROM <a style="font-weight: bold;"><?php echo "$startdate2"; ?></a> TO <a style="font-weight: bold;"><?php echo "$enddate2"; ?></a> </span>
                </h5>

                <?php 

                $expen ="SELECT * FROM expenditure_table WHERE category='expenses' AND request_status='approved' AND expense_date BETWEEN '$startdate2' AND '$enddate2' "  ;
                $expresult = mysqli_query($connect, $expen);

                //echo"$expen";
                while($row=mysqli_fetch_array($expresult)){
                   $naam =$row['amount_spent'];
                  // echo "$naam";
                }
                 ...
                 $intt ="SELECT total_income,costofsales FROM billsheet WHERE datebilled BETWEEN '$startdate2' AND '$enddate2'  and cash_category='internal' ";
                 $riz =mysqli_query($connect, $intt);
                 $tot_int =0;
                 $costofsales =0;
                 while($row=mysqli_fetch_array($riz)){
                    $income =$row['total_income'];
                    $costofsaless = $row['costofsales'];
                    $tot_int =$tot_int + $income;
                    $costofsales =$costofsales + $costofsaless;
                  
                 }

                 $inttt ="SELECT total_income FROM billsheet WHERE datebilled BETWEEN '$startdate2' AND '$enddate2'  and cash_category='external' AND caseid_status='expired' AND cashtype !='investment' ";
                 $rizz =mysqli_query($connect, $inttt);
                 $tot_other =0;
                 while($row=mysqli_fetch_array($rizz)){
                    $income =$row['total_income'];
                    $tot_other =$tot_other + $income;
                  
                 }

              
               
                 $ser = "SELECT * FROM billsheet WHERE datebilled BETWEEN '$startdate2' AND '$enddate2' ";
                 ...
                 echo"<table class='table table-striped table-sm '>
                    ...
                    $id =$row['bill_id'];
                    $category = $row['cash_category'];
                    $paydate =$row['datebilled'];
                    $details =$row['cashtype'];
                 ...
                 $grossprofit =$tot_int - $costofsales;
                 $bal = $grossprofit + $tot_other;
                ...
                $expenses ="SELECT  SUM(amount_spent),department,expense_date,ex_id FROM expenditure_table WHERE category='expenses' AND request_status='approved' AND expense_date BETWEEN '$startdate2' AND '$enddate2' GROUP BY department  ";
                $real =mysqli_query($connect, $expenses)or die(mysqli_error($connect));
                $tox =0;
                while($row=mysqli_fetch_array($real)){
                    $id =$row['ex_id'];
                    $used =$row[0];
                    $tox =$tox + $used;
                   // echo "<script>alert('$expenses')</script>";
                  // $man =$row['amount_spent'];
                   //print_r($row);
                   // echo"$row[1]".":";
                   ...
                   echo "<thead>
                            <tr>
                                ...
                                <th scope='col'>Total Expenses(GHC)</th>
                                <th scope='col'> $tox</th>
                                <th scope='col'> $tox</th>
                            </tr>
                        </thead>";
                     echo"   <thead>
                        ...
                        <tr>
                            <th>#</th>
                            <th scope='col'>Net Profit(GHC)</th>
                            <th scope='col'>--</th>
                            <th scope='col'>$Netprofit</th>
                        </tr>
