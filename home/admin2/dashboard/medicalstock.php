...
if(isset($_POST['income_schedule'])){
  ...
  $ser = "SELECT * FROM cash_inflow_book WHERE income BETWEEN '$startdate1' AND '$enddate1' ";
...
<div class="container-fluid">
 ...
 <main class="col-md-9 ms-sm-auto col-lg-10 " id="income_schedule" style="">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h5 class="tees">Drug Stock</h5>
    ...
    <div class="row">
        ...
        <div class="row" style="margin-left:0.2%;margin-bottom: 2%;">
            <div class="card border-left-primary shadow h-100 py-2 studdlist">
                <pre></pre>
                <h5 class="payroll">
                    <span>Drug Stock</span><br>
                    <span>FROM <a style="font-weight: bold;"><?php echo"$startdate2" ?></a> TO <a style="font-weight: bold;"><?php echo"$enddate2" ?></a> </span>
                </h5>

                <?php 
                 $serr = "SELECT * FROM stock_table WHERE invetdate BETWEEN '$startdate2' AND '$enddate2'  UNION SELECT * FROM stock_table WHERE invetdate <'$startdate2' AND stocktype='medical_stock'  ";
                    $resultt =mysqli_query($connect, $serr)or die(mysqli_error($connect));
                   ...
                   // print_r($clss_cash);
                    ...
                    $sower = "SELECT SUM(quantity), quantity,item_name,item_id,purchaseddate,status,supplier,category,groupp,unit_cp,unit_sp,tot_cp,tot_sls,expirydate,invoicenumber,dosage FROM medstock_table WHERE invetdate BETWEEN '$startdate2' AND '$enddate2' AND status='expired' GROUP By item_name,dosage,category";
                    $rower =mysqli_query($connect, $sower)or die(mysqli_error($connect));
                     ...
                     <thead>
                      <tr>
                       ...
                       <th scope='col'>Mls</th>
                       <th scope='col'>OPS (Qty)</th>
                       <th scope='col'>New Purchase(Qty)</th>
                       <th scope='col'>Qty Sold</th>
                       <th scope='col'>Unit C.P</th>
                       <th scope='col'>Unit S.P</th>
                       <th scope='col'>T. CP(cash)</th>
                       <th scope='col'>T. sld (cash)</th>
                       <th scope='col'>CLS (Qty)</th>
                       <th scope='col'>CLS (Cash)</th>
                      </tr>
                      </thead>";

                      if(mysqli_num_rows($rower)<1){
                    ...
                    while ($row=mysqli_fetch_array($rower)) {
                        ...
                        $namnam =json_decode($nam,true);
                        $sumTotal =$row[0];
                       // $sumTotal = $sumTotal + $qty;

                           if($nam){
                            //echo "$nam <br>";
                        $moro ="SELECT * FROM medstock_table WHERE invetdate <'$startdate2' AND item_name='$nam' ";
                        $morreso =mysqli_query($connect,$moro) or die(mysqli_error($connect));
                        
                        while ($row =mysqli_fetch_array($morreso)) {
                            $tqty =$row['quantity'];
                            $names =$row['item_name'];
                            $ml =$row['dosage'];
                           // echo " paul $tqty paul";
                 ...
                 $ser = "SELECT * FROM stock_table WHERE stocktype='medical_stock' AND invetdate BETWEEN '$startdate2' AND '$enddate2'  ";
                 ...
                 echo"<table class='table table-striped table-sm '>
                    ...
                    $id =$row['item_id'];
                    $invetdate =$row['invetdate'];
                    $itemname =$row['item_name'];
                    $purchaseddate =$row['purchaseddate'];
                    $opscash = '300';
                    $openstock =$row['ops_qty'];
                    $newqtystock =$row['ops_qty'];
                    $qtysold =$row['qtysold'];
                    $unitcost =$row['unit_cp'];
                    $unitsell =$row['unit_sp'];
                    $totcost = $row['tot_cp'];
                    $oopscashh =$row['cls_cash'];
                    $totsales =$qtysold * $unitsell;
                    $closingstock = $openstock + $newqtystock - $qtysold;
                    $clscash =$opscash+$totcost-$totsales;
                    $ver =$unitcost * $closingstock;
                    //$cum = $cum + $amount;
                    if($itemname){

                        $mor ="SELECT * FROM stock_table WHERE invetdate <'$startdate2' AND stocktype='medical_stock' ";
                        $morres =mysqli_query($connect,$mor) or die(mysqli_error($connect));
                        while ($rrow =mysqli_fetch_array($morres)) {
                            $tty =$row['cls_cash'];
                        }
                   ...
                   $income =$totsales;
                   $total_revenue =$total_revenue + $income;
                   }
                 }
                   echo "<thead>
                            <tr>
