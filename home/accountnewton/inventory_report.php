...
if (isset($_POST['inventory_rep'])) {
	...
	$sql2 = "SELECT stock_date, drug_name, quantity, expiry_date 
	FROM drug_inventory 
...
if ($result) {
 ...
 <body class="">
 	...
 	<form method="POST" class="form-group text-white">
 		...
 		<br>
 		<input type="submit" name="inventory_rep" value="Get Report" onclick="Alert()">
 	</form>
 	</center>
  ...
  <tbody>
  	<?php 
  		...
  		foreach ($report as $key => $value) {
  			...
  			$date = $value['stock_date'];
  			$name = $value['drug_name'];
  			$categ = $value['quantity'];
  			$amt = $value['expiry_date'];
  			#$total += $amt;
  			$counter +=1;
  			echo "
  		<tr>
